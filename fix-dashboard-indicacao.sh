...
docker exec crm-backend-1 mysqldump -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" > "$BACKUP_FILE"
...
docker exec -i crm-backend-1 mysql -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME" < backend/migrations/03-indicadores-saques.sql
...
if [ $? -eq 0 ]; then
    ...
    echo "docker exec -i crm-backend-1 mysql -u\$DB_USER -p\$DB_PASSWORD \$DB_NAME < $BACKUP_FILE"
    exit 1
fi
...
echo "  http://185.217.125.72:3000/admin?view=dashboard-indicacao"
echo ""
echo "📊 Logs do backend (se necessário):"
echo "  docker logs crm-backend-1 --tail 50 -f"
echo ""
