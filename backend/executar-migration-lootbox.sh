...
echo "Executando migration para adicionar lootbox..."
cd "$(dirname "$0")"

# Tentar diferentes formas de executar o MySQL
if command -v mysql &> /dev/null; then
    mysql -u root protecar_crm < migrations/schema-lootbox.sql
elif [ -f "/usr/bin/mysql" ]; then
    /usr/bin/mysql -u root protecar_crm < migrations/schema-lootbox.sql
else
