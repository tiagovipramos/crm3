...
# Identificar o nome do container MySQL
...
cat backend/migrations/03-indicadores-saques.sql | docker-compose exec -T mysql mysql -u"$DB_USER" -p"$DB_PASSWORD" "$DB_NAME"
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/api/admin/estatisticas/indicacao -H "Authorization: Bearer test")
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/api/admin/top-indicadores -H "Authorization: Bearer test")
if [ "$RESPONSE" = "401" ] || [ "$RESPONSE" = "200" ]; then
    echo "✅ OK (Status: $RESPONSE)"
else
    echo "⚠️  Status: $RESPONSE"
...
# Verificar alertas
echo -n "  - Alertas... "
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/api/admin/alertas -H "Authorization: Bearer test")
if [ "$RESPONSE" = "401" ] || [ "$RESPONSE" = "200" ]; then
    echo "✅ OK (Status: $RESPONSE)"
else
    echo "⚠️  Status: $RESPONSE"
...
RESPONSE=$(curl -s -o /dev/null -w "%{http_code}" http://localhost:3001/api/admin/saques/pendentes -H "Authorization: Bearer test")
if [ "$RESPONSE" = "401" ] || [ "$RESPONSE" = "200" ]; then
    echo "✅ OK (Status: $RESPONSE)"
else
    echo "⚠️  Status: $RESPONSE"
